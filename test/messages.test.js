...
import { composeNotificationMessage, summarizeSightings } from '../src/messages.js';

const buildTimestamps = (now, offsets) => offsets.map((offset) => now - offset);

test('summarizeSightings tallies recent activity windows', () => {
  const now = Date.now();
  const timestamps = buildTimestamps(now, [500, 2_000, 3_000, 50_000, 200_000, 2_000_000]);
...
test('composeNotificationMessage references frequent visitors', () => {
  const now = Date.now();
  const recentOffsets = Array.from({ length: 4 }, (_, idx) => idx * 10 * 60 * 1000); // every 10 minutes
